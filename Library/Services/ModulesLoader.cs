...
public class ModulesLoader : IModulesLoader
{
    ...
    public void Load(Action<Assembly> hookCallback)
    {
        ...
        if (_options == null)
        {
            _logger.LogError("No module options were found, but there are {count} modules present. Modules will not be loaded.", _manfiests.Count);
            return;
        }

        if (_options.LoadOrder == null || _options.LoadOrder.Length == 0)
        {
            _logger.LogError("Module load order is not specified, but there are {count} modules present. Modules will not be loaded.", _manfiests.Count);
            return;
        }
        ...
        {
            ParsedFile<ModuleManfiest>? manifestFile = _manfiests.FirstOrDefault(manfiest => manfiest.Value.ID == id);
            ...
            ModuleManfiest manifest = manifestFile.Value;
    ...
    private static Result<Exception?> LoadModule(Action<Assembly> hookCallback, ILogger logger, IFileService fileService, ModuleOptions options, ModuleManfiest manifest, IPath directory)
    {
